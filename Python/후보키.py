...
def check(num, checked_list):
    for checked in checked_list:
        result = True
        for tmp in checked:
            result = result and (tmp in num)
        if result:
            return False
    return True
...
def solution(relation):
    # 1. 각각 가능한 모든 조합을 만들어 낸다.
    # 2. 앞에서부터 하나씩 비교하여 중복이 발생하지 않는 경우를 찾아낸다.
    NUM_COLUMNS = len(relation[0])
    ...
    for row in range(0, NUM_ROWS):
        ...
        for column in range(1, NUM_COLUMNS + 1):
            temp.extend(
                list(combinations(relation[row], column)))
        all_list.append(temp)
    cols = [str(x) for x in range(1, NUM_COLUMNS + 1)]
    num_cols = []
    for num in range(1, NUM_COLUMNS + 1):
        num_cols.extend(map("".join, combinations(cols, num)))

    tmps = list(zip(*all_list))
    checked_list = []
    for idx in range(len(tmps)):
        if len(tmps[idx]) == len(set(tmps[idx])) and check(num_cols[idx], checked_list):
            checked_list.append(num_cols[idx])
